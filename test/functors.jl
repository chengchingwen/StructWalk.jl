using StructWalk: FunctorStyle, fmap, isleaf, NoChildren, children, fmapstructure
using StructWalk
using Test

const FS = FunctorStyle()
...
StructWalk.children(::FunctorStyle, a::OneChild3) = (a.y,)
StructWalk.constructor(::FunctorStyle, a::OneChild3) = ch -> OneChild3(a.x, ch..., a.z) 

struct NoChildren2; x; y; end
StructWalk.children(::FunctorStyle, a::NoChildren2) = ()
StructWalk.constructor(::FunctorStyle, a::NoChildren2) = _ -> a

struct NoChild{T}; x::T; end
StructWalk.children(::FunctorStyle, a::NoChild) = ()
StructWalk.constructor(::FunctorStyle, a::NoChild) = _ -> a
...
@testset "Children and Leaves" begin
  ...
  @test isleaf(no_children)
  @test !isleaf(has_children)
  @test children(FS, no_children) === NoChildren()
  @test children(FS, has_children) == (x=1, y=2)
end
